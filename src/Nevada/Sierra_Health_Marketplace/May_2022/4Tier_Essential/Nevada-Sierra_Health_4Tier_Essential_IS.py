...
LOG_FILE = 'Nevada_Sierra_Health_4Tier_essential_May22.log'
BASE_DATA_FILE = '4 tier Essential IS.pdf'
JSON_FILE = 'Nevada_Sierra_Health_4tier_essential_May_22.json'
PROCESSED_DATA_FILE = 'Nevada_Sierra_Health_4tier_essential_May_22.csv'
RXNORM_MAPPING_FILE = 'Nevada_Sierra_Health_4tier_essential-rxnormid_drugs-May_22.txt'
...
# arguments: detected table in the given region of the page
def parse_table_first_iteration(list_detected_tables, hlines_base):
    list_horizontal_lines = [hlines_base]
        ...
        with pdfplumber.open(path) as pdf:
            for i in page_range:
                try:
                    ...
                    list_horizontal_lines_left = parse_table_first_iteration(
                        page_content_left.find_tables(table_config_left.table_settings), 
                        hlines_left[0])
                    ...
                    list_horizontal_lines_right = parse_table_first_iteration(
                        page_content_right.find_tables(table_config_right.table_settings), 
                        hlines_right[0])
...
def process_file_content(df_file_content):
    ...
    # Keep only the rows where the column DrugTier is in ['1', '2', '3', '4', 'NF']
    df_file_content = df_file_content[df_file_content.DrugTier.isin(['1', '2', '3', '4', 'NF'])].reset_index(drop=True)
...
def build_urls(drugname):
    drug_types = ['SOLR', 'TABS', 'CAPS', 'SOLN', 'TB24', 'SUSR', 'SUSP', 'CHEW', 'PACK', 'POWD', 'CONC', 
        'CPEP', 'CP12', 'GRAN', 'CP24', 'PT24', 'CREA', 'CSDR', 'LIQD', 'SOAJ', 'PTTW', 'ELIX', 'HOUR', 
        'SYRP', 'ENEM', 'CPCR', 'chew', 'OINT', 'LOTN', 'GEL', 'TBCR', 'PNKT', 'tbcr or', 'solg', 'TABLET',
        'lpop bu', 'pt72 td', 'T12A', 'TB12', 'subl sl', 'FILM', 'subl', 'ptwk td', 'nebu in', 'SOSY', 'RELEASE',
...
if __name__ == '__main__':
    ...
    # list_header_cols = ['Drug Name', 'Drug Tier', 'Requirements \n& Limits']
    list_cols = ['DrugName', 'DrugTier', 'Requirements_Limits']
    page_range = range(7, 39)
    try:
        # Load the table configuration for the pdf file
