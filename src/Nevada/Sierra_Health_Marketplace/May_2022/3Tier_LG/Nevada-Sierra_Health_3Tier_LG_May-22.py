...
LOG_FILE = 'Nevada-Sierra_Health_3Tier_LG_May22.log'
BASE_DATA_FILE = '3 tier LG.pdf'
JSON_FILE = 'Nevada-Sierra_Health_3Tier_LG_May_2022.json'
PROCESSED_DATA_FILE = 'Nevada-Sierra_Health_3Tier_LG-May_22.csv'
RXNORM_MAPPING_FILE = 'Nevada-Sierra_Health_3Tier_LG-rxnormid_drugs-May_22.txt'
...
if provider(excluded=['alibaba', 'aws', 'do', 'azure', 'oci']) == 'gcp':
    ...
    BASE_OUTPUT_DIR = '../../../../../output/Nevada/Sierra_Health/May_2022'
    BASE_DATA_DIR = '../../../../../data/Nevada/Sierra_Health/May_22'
    IS_ON_GCP = False
...
def get_header_index(table, table_headers):
    ...
    for row in table:
        if len(row) == 4:
            ...
            if (list_header_cols[0] in row[0]) and (list_header_cols[1] in row[1]) and (list_header_cols[2] in row[2]) and (list_header_cols[3] in row[3]):
                return header_row
            else:
                header_row += 1
        ...
        with pdfplumber.open(path) as pdf:
            for i in page_range:
                try:
                    ...
                    # table_content = page_content.extract_table(table_config.table_settings)
                    # # reset the table_settings explicit_horizontal_lines & horizontal_strategy
                    # table_config.set_hlines(hlines)
                    # table_config.set_hstrategy("lines")
                    ...
                    # Removing the search for header index since in some pages the table starts with data instead of headers. 
                    # Loading all rows into the DataFrame and doing the post processing
                    ...
                    logger.critical("Error at page: {} - {}".format(i, e))
                    pass
    except Exception as e:
...
def process_file_content(df_file_content):
    # Build a column DrugName by appending the columns Brand Name and Generic Name like this: 'Brand Name (Generic Name)'
    # If any of the columns Brand Name or Generic Name is NaN, then keep the other column value
    ...
    # Keep only the rows where the column DrugTier is in ['1', '2', '3']
    df_file_content = df_file_content[df_file_content.DrugTier.isin(['1', '2', '3'])].reset_index(drop=True)
    ...
    file_content['DrugName'] = file_content['DrugName'].replace("\*", "", regex=True)
    # Remove all extra spaces from the column DrugName
    ...
    for (index, row) in file_content.iterrows():
        ...
        for url in row[3]:
            try:
                ...
                logger.exception("Drug name: {}, response: {}, Exception: {}".format(row[0]+": " + url, response.json(), e))
                _dict["rxnorm_id"] = ""
