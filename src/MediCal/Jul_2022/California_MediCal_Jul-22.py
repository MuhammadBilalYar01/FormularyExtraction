...
LOG_FILE = 'CA_Medical_Medicaid_Jul_22.log'
BASE_DATA_FILE = 'Medi-Cal_Rx_Contract_Drugs_List_FINAL.pdf'
JSON_FILE = 'CA_Medical_Medicaid_Jul_22.json'
PROCESSED_DATA_FILE = 'CA_Medical_Medicaid_Jul_22.csv'
RXNORM_MAPPING_FILE = 'CA_Medical_Medicaid-rxnormid_drugs-Jul_22.txt'
...
if provider(excluded=['alibaba', 'aws', 'do', 'azure', 'oci']) == 'gcp':
    ...
    BASE_DATA_DIR = '../../../data/MediCal/07-22/'
...
def process_file_content(df_file_content):
    ...
    # Drop the rows wehre the column BillingUnit is not in [np.nan, 'ea', 'ml', 'm', 'Ml', 'gm', 'each', 'Each', '60each', 'capsule', 'gram','package', 'kit'] or the column BillingUnit is a number
    df_file_content = df_file_content[(df_file_content.BillingUnit.isin(
        [np.nan, 'ea', 'ml', 'm', 'Ml', 'gm', 'each', 'Each', '60each', 'capsule', 'gram','package', 'kit'])) | (df_file_content['BillingUnit'].str.isdigit())].reset_index(drop=True)
    ...
    index_drug_overflow_5 = list(df_file_content[(df_file_content['DrugName'] == 'and Salmeterol *') & 
                        (df_file_content['Dosage'] == 'inhalation')].index)
    for index in index_drug_overflow_5:
        if str(df_file_content.iloc[index - 1, 0]) != 'nan':
            df_file_content.iloc[index - 1, 0] += ' ' + df_file_content.iloc[index, 0]
            df_file_content.iloc[index, 0] = np.nan
...
overflow_text_dosages= ['suspension','injection','Suspension Pen','solution *','injectable','solution/drops', 'Release Injectable', 
                        ...
                        'premixed','inhalation with','inhalation device','Inhalation *','(without','without','as the propellant','inhalation with','inhalation device','disintegrating +','Release +','long-acting +',
...
overflow_text_strength2=['PFU/ml','ampule', 'per inhalation', 'per inhalation,']
    ...
    for (index, row) in file_content.iterrows():
        ...
        if row[1] in overflow_text_dosages:
            ...
            if row[2] is not np.NaN:
                join_char = ' ' if str(file_content.iloc[index, 2]).endswith(('/', ',')) else '; '
                file_content.iloc[index, 2] = row[2] if pd.isna(file_content.iloc[index, 2]) else  join_char.join([str(file_content.iloc[index, 2]), row[2]])
            if row[5] is not np.NaN:
                file_content.iloc[index, 5] = row[5] if pd.isna(file_content.iloc[index, 5]) else  ' '.join([str(file_content.iloc[index, 5]), row[5]])
                # file_content.iloc[index, 5] = ' '.join([str(file_content.iloc[index, 5]), row[5]])
            if (row[3] is not np.NaN) and (str(file_content.iloc[index, 3]).strip() != str(row[3]).strip()):
...
## [START] Function to validate the processed data
def validate_processed_data(file_content):
    nan_drugname_rows = file_content.loc[file_content['DrugName'].str.contains("nan")]
    if len(nan_drugname_rows) > 0:
        logger.error("Found rows with nan in DrugName column:\n{}".format(nan_drugname_rows))
    nan_dosage_rows = file_content.loc[file_content['Dosage'].str.contains("nan", na = False)]
    if len(nan_dosage_rows) > 0:
        logger.error("Found rows with nan in Dosage column:\n{}".format(nan_dosage_rows))
    nan_strength_rows = file_content.loc[file_content['Strength'].str.contains("nan", na = False)]
    if len(nan_strength_rows) > 0:
        logger.error("Found rows with nan in Strength column:\n{}".format(nan_strength_rows))
## [END] Function to  validate the processed data
...
def fetch_rxnorm_id_row(file_content_row):
    ...
    for url in file_content_row[7]:
        try:
            ...
            logger.exception("Drug name: {}, response: {}, Exception: {}".format(file_content_row[6]+": " + url, response.json(), e))
            file_content_row[9] = ""
...
if __name__ == '__main__':
    ...
    try:
        ...
        validate_processed_data(processed_file_content)
