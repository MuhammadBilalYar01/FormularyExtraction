...
LOG_FILE = 'Florida_Oscar_Medicare_Adv_HFHP_Apr_22.log'
BASE_DATA_FILE = 'FL_Oscar_Medicare_Adv_HFHP_Apr_22.pdf'
JSON_FILE = 'Florida_Oscar_Medicare_advantage_HFHP_Apr_22.json'
PROCESSED_DATA_FILE = 'Florida_Oscar_Medicare_adv_HFHP_Apr_22.csv'
RXNORM_MAPPING_FILE = 'Florida_Oscar_Medicare_adv_HFHP-rxnormid_drugs-Apr_22.txt'
...
def fetch_rxnorm_ids(file_content_chunk):
    with concurrent.futures.ThreadPoolExecutor(max_workers=NUM_THREADS) as thread_executor:
        ...
        future_rxnormid_content = {
                                    thread_executor.submit(fetch_rxnorm_id_row, file_content_row): file_content_row for (_, file_content_row) in file_content_chunk.iterrows() 
                                }
    ...
    for generator_row in file_content:
        ...
        for row in generator_row:
            try:
                ...
                if row[2] != row[2]: # if row[2] is np.NaN:
...
if __name__ == '__main__':
    ...
    drug_types = ['SOLR', 'TABS', 'CAPS', 'SOLN', 'TB24', 'SUSR', 'SUSP', 'CHEW', 'PACK', 'POWD', 'CONC', 
              ...
              'PSKT','AERS','NEBU','AERB','TBPK','CART','TBDD','PNKT','LQPK','TBSO']
    drug_types.extend([x.lower() for x in drug_types])
    page_range = range(6, 78)
    try:
